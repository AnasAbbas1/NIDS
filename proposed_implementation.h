...
private:
    static ull* Step1(){
        ull* ret = NULL;
        ull* h_lookupTabe = new ull [h_mps[h_masksz]];
        ull currents[] = {1, 1, 1};
        for(int i = 0; i < h_mps[h_masksz]; i++){
            h_lookupTabe[i] = 0;
        ...
        CubDebugExit(g_allocator.DeviceAllocate((void**)&ret, sizeof(ull) * h_mps[h_masksz]));
        CubDebugExit(cudaMemcpy(ret, h_lookupTabe, sizeof(ull) * h_mps[h_masksz], cudaMemcpyHostToDevice));
        delete[] h_lookupTabe;
